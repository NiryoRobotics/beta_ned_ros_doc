...
class AbstractRosdocDirective(SphinxDirective, abc.ABC):
    ...
    def run_rosdoc_lite(self, package_path: str):
        ...
        output_path = os.path.join(self.env.app.outdir, 'rosdoc_lite',
                                   os.path.basename(package_path))
        subprocess.run(["rosdoc_lite", package_path, "-o", output_path],
                       check=True)
    ...
    def resolve_file_path(self, relative_path: str) -> str:
        ...
        if not os.path.isfile(file_path):
            raise self.error(
                f"The path '{relative_path}' (resolved to '{file_path}') does not exist or is not a file."
            )
        return file_path
    ...
    def add_link_to_type_documentation(self,
                                       interface_type: str,
                                       interface_category: str = "msg") -> str:
        """
    ...
    def render_template(self,
                        ...
                        template_dir: str,
                        template_file: str,
                        helper_functions: dict = None) -> str:
        """
